    ...
    for (i in 1:nrow(ngvc))
    {
      ...
      if (ent[i] > 0.5)
        ...
        else
        {
          ...
          s <- paste0("p=",1)
          ests <-
            paste0(sig," ","entropy=",0," ",s,"-no neighbors in graph.")
...
example("/Users/josephkeshet/Desktop/I.Rda")
X <- as.matrix(c(1,2,3,1,2,3),ncol = 1)
Z <- as.matrix(c(1,2,3,1,2,3),ncol = 1)
Y <- as.matrix(c(1,2,3,6,5,4),ncol = 1)
x <- as.matrix(c(2.5,3.8,5),ncol = 1)
z <- x
Y<-2*X+1
as.data.frame((cbind(Y,Y,Z))) -> df
colnames(df) <- c("X","Y","Z")
gmodel <- kinn(Y ~ X,df)
as.data.frame((cbind(x,z))) -> df2
colnames(df2) <- c("X","Z")
predict.kinn(gmodel,df2) -> ygraph
plot(Y,Y,xlim = c(-20,20))
points(cbind(as.vector(x),ygraph),pch = 8)
#plotGraphs(gmodel)
